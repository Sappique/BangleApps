if (combiclockTimerTimeout) clearInterval(combiclockTimerTimeout);
...
function showTimer(timer) {
  ...
  E.showPrompt("Timer finished!",{
    ...
    buzzCount = 0;
    timer.start = null;
    require("Storage").write("combiclock.timer.json",JSON.stringify(timers));
    load();
  });
...
if (active.length) {
  ...
  active = active.sort((a,b)=>combiclock.timerExpiresIn(a)-combiclock.timerExpiresIn(b));
  showTimer(active[0]);
} else {
