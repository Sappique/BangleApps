...
var all = require("Storage").readJSON("combiclock.timer.json") || [];
...
function update() {
  ...
  if (all[current].start) {
    drawIntervalTimeout = setTimeout(() => {drawInterval = setInterval(draw, 1000); draw();}, 1000 - (combiclock.getTime(all[current]) % 1000));
  } else {
    drawInterval = null;
...
function play() {
  ...
  require("Storage").write("combiclock.timer.json",JSON.stringify(all));
...
function draw() {
  ...
  var time = all[current].time - combiclock.getTime(all[current]);
  ...
  var timeStr = combiclock.formatTime(time, false, false, true);
