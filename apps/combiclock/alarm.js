...
var all = require("Storage").readJSON("combiclock.alarm.json") || [];
...
function activate() {
  ...
  update();
  require("Storage").write("combiclock.alarm.json",JSON.stringify(all));
...
var buttons = {
  reset: {pos:[0, height-settings.buttonHeight, width/2, height], callback: remove, img: combiclock.remove_img, col:"#f50"}, // remove
  play: {pos:[width/2, height-settings.buttonHeight, width, height], callback: activate, img: combiclock.play_img, col:"#0ff"}, // active
};
...
function draw() {
  ...
  var timeStr = combiclock.formatTime(all[current].time, false, false, true);
