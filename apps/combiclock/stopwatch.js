...
var all = require("Storage").readJSON("combiclock.stopwatch.json") || [];
...
function update() {
  ...
  var interval = Math.floor(combiclock.getTime(all[current])/3600000)?1000:100;
  if (all[current].start) {
    drawIntervalTimeout = setTimeout(() => {drawInterval = setInterval(draw, interval); draw();}, interval - (combiclock.getTime(all[current]) % interval));
  } else {
    drawInterval = null;
...
function play() {
  ...
  require("Storage").write("combiclock.stopwatch.json",JSON.stringify(all));
}
function reset() {
...
var buttonsRunning = {
  reset: {pos:[0, height-settings.buttonHeight, width/2, height], callback: reset, img: combiclock.reset_img, col:"#f50"},
  play: {pos:[width/2, height-settings.buttonHeight, width, height], callback: play, img: combiclock.play_img, col:"#0ff"},
};
var buttonsNormal = {
  reset: {pos:[0, height-settings.buttonHeight, width/2, height], callback: remove, img: combiclock.remove_img, col:buttonsRunning.reset.col},
  play: {pos:[width/2, height-settings.buttonHeight, width, height], callback: play, img: combiclock.play_img, col:buttonsRunning.play.col},
};
...
function drawButtons() {
  if (all[current].start || all[current].time) {
    ...
    if (all[current].start) {
      buttons.play.img = combiclock.pause_img;
    } else {
      buttons.play.img = combiclock.play_img;
    }
  } else {
    buttons = buttonsNormal;
  }
...
function draw() {
  ...
  var timeStr = combiclock.formatTime(combiclock.getTime(all[current]), false, true);
...
combiclock.registerControls(this);
